...
data[2].data.forEach(async el => {
    await db.execute('INSERT INTO \`case\` values(?,?,?)',[el.case_id,el.case_type,el.case_description]);
});
data[3].data.forEach(async el => {
    await db.execute('INSERT INTO prison values(?,?,?,?)',[el.prison_id,el.prison_name,el.address,el.warden]);
});
